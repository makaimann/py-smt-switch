...
def _partial_eval(fun, m, *args, **kwargs):
    ...
    if fun.__class__ == mypartial:
        args = fun.args + args
        kwargs = {**fun.keywords, **kwargs}

    if len(args) < m:
        return mypartial(fun, *args, **kwargs)
        ...
        if args:
            ...
            return s_terms[0].solver.apply_fun(mypartial(fun, args[:m-1]), *args[m:], **kwargs)
